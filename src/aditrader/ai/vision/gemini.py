"""Google Gemini Multimodal Vision Engine implementation.

Per Phase 7B, ADR 005, and ADR 012:
- Analyzes market chart screenshots to extract structural market features.
...
- Bounded execution: zero automatic retries that could burn API credits.
- All provider/network errors mapped into typed AIError hierarchy.
"""
...
# Transport signature: (url, payload, timeout_seconds) -> response_dict
TransportCallable = Callable[[str, dict[str, Any], float], dict[str, Any]]


def default_gemini_http_transport(
    url: str,
    payload: dict[str, Any],
    timeout_seconds: float,
) -> dict[str, Any]:
    """Default HTTP transport calling Gemini REST API via standard library urllib."""
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout_seconds) as response:
            ...
            if not isinstance(parsed, dict):
                raise AIMalformedOutputError(
                    f"Expected JSON object from provider response, got {type(parsed).__name__}"
                )
            return parsed
        ...
        if e.code in (401, 403):
            raise AICredentialError(
                f"Gemini API authentication failed (HTTP {e.code}): {err_body}"
            ) from e
        if e.code == 404:
            raise AIModelNotFoundError(
                f"Gemini model endpoint not found (HTTP 404): {err_body}"
            ) from e
        if e.code == 429:
            raise AIProviderError(
                f"Gemini API rate limit or quota exceeded (HTTP 429): {err_body}"
            ) from e
        if e.code >= 500:
            raise AIUnavailableError(
                f"Gemini API service temporarily unavailable (HTTP {e.code}): {err_body}"
            ) from e
        raise AIProviderError(f"Gemini API HTTP error (HTTP {e.code}): {err_body}") from e
    except (urllib.error.URLError, TimeoutError) as e:
        if isinstance(e, TimeoutError) or "timed out" in str(e).lower():
            raise AITimeoutError(
                f"Gemini API request timed out after {timeout_seconds:.1f}s"
            ) from e
        raise AIUnavailableError(f"Gemini API connection error: {e}") from e
    except json.JSONDecodeError as e:
        raise AIMalformedOutputError(f"Failed to decode Gemini HTTP response as JSON: {e}") from e


class GeminiVisionEngine(VisionEngine):
    """Concrete VisionEngine implementation using Google Gemini multimodal models."""
    ...
    def is_available(self) -> bool:
        """Return True if model credentials are configured and provider is available.
    ...
    def analyze(
        ...
        """Extract observable support/resistance and chart patterns from image bytes.
        ...
        Raises:
            ...
            AIMalformedOutputError: If model output is missing, corrupted, or violates bounds.
        ...
        # Execute optional OCR if not supplied and OCR engine is available
        if ocr_result is None and self.ocr_engine is not None and self.ocr_engine.is_available():
            try:
                ...
                # OCR failure must NOT fail vision analysis
                ocr_result = None
        ...
        transport_fn = self._transport or default_gemini_http_transport
        # Single bounded call: no retry loop
        response_data = transport_fn(url, payload, self.timeout_seconds)
    ...
    def _parse_and_validate_response(
        ...
        """Parse structured model response, validate against domain rules, and build VisionResult."""
        if not isinstance(response_data, dict):
            raise AIMalformedOutputError(
                f"Expected dictionary from provider response, got {type(response_data).__name__}"
            )

        candidates = response_data.get("candidates")
        if not candidates or not isinstance(candidates, list) or len(candidates) == 0:
            raise AIMalformedOutputError("Gemini response contains no candidates")

        candidate = candidates[0]
        if not isinstance(candidate, dict):
            raise AIMalformedOutputError("Candidate entry is not a dictionary")

        finish_reason = candidate.get("finishReason")
        if finish_reason in ("SAFETY", "RECITATION", "BLOCKLIST", "OTHER") and not candidate.get(
            "content"
        ):
            raise AIMalformedOutputError(
                f"Gemini candidate terminated prematurely with finishReason: {finish_reason}"
            )

        content = candidate.get("content")
        if not content or not isinstance(content, dict):
            raise AIMalformedOutputError("Candidate missing content dictionary")

        parts = content.get("parts")
        if not parts or not isinstance(parts, list) or len(parts) == 0:
            raise AIMalformedOutputError("Candidate content contains no parts")

        first_part = parts[0]
        if not isinstance(first_part, dict) or "text" not in first_part:
            raise AIMalformedOutputError("Candidate part does not contain 'text' key")

        raw_text = first_part["text"]
        if not isinstance(raw_text, str) or not raw_text.strip():
            raise AIMalformedOutputError("Candidate part text is empty")
        ...
        try:
            ...
            raise AIMalformedOutputError(
                f"Failed to parse Gemini model output as JSON: {exc}"
            ) from exc
        ...
        # 7. Model version (extracted from provider response if exposed; fallback to configured version or model_id)
        raw_version = response_data.get("modelVersion")
        model_version: str
    ...
    def _build_prompt(
        ...
        """Construct structured multimodal prompt instructing Gemini to output pure JSON."""
        prompt = (
